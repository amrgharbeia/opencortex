#+PROPERTY: header-args:lisp :tangle (expand-file-name "act.lisp" (or (identity (getenv "INSTALL_DIR")) (file-name-directory (buffer-file-name))))
...
#+begin_src lisp :tangle (expand-file-name "pipeline-act-tests.lisp" (concat (or (identity (getenv "INSTALL_DIR")) ".") "/tests"))

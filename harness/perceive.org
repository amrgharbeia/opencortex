#+PROPERTY: header-args:lisp :tangle (expand-file-name "perceive.lisp" (or (identity (getenv "INSTALL_DIR")) (file-name-directory (buffer-file-name))))
...
#+begin_src lisp :tangle (expand-file-name "pipeline-perceive-tests.lisp" (concat (or (identity (getenv "INSTALL_DIR")) ".") "/tests"))
